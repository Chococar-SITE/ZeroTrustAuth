...
final class FileLogSink implements LogSink {
    ...
    /** 目前 active 檔對應的「日」（UTC），用於偵測跨日輪替。 */
    private LocalDate currentDay;
    private boolean closed;

    /**
     * @param dataFolder    外掛資料夾（{@code getDataFolder()}）
     * @param retentionDays 已輪替檔保留天數（計劃預設 90；&lt;= 0 表示不清除）
     * @param log           外掛 logger（記錄 IO 警告，不含秘密）
     */
    FileLogSink(Path dataFolder, int retentionDays, Logger log) {
        ...
        try {
            ...
            // 啟動期 IO 失敗：包成 unchecked 讓 SelfTest / onEnable 能將其視為 fail-closed 訊號。
    ...
    /** 從歸檔檔名 {@code zerotrust-YYYY-MM-DD[...].log} 解析日期；無法解析回傳 null。 */
    private static LocalDate parseArchiveDate(Path p) {
        ...
        // 取前 10 字元（yyyy-MM-dd），忽略可能附加的 -timestamp 後綴。
        if (middle.length() < 10) {
            return null;
        }
        try {
