...
/**
 * {@link Scheduler} 的 NeoForge 實作（計劃 2.3）。
 *
 * <h2>執行緒模型</h2>
 ...
 */
final class NeoForgeScheduler implements Scheduler {

    private final MinecraftServer server;
    private final Logger log;
    private final ScheduledExecutorService timer;

    NeoForgeScheduler(MinecraftServer server, Logger log) {
        this.server = Objects.requireNonNull(server, "server");
        this.log = Objects.requireNonNull(log, "log");
        ScheduledThreadPoolExecutor exec = new ScheduledThreadPoolExecutor(1, r -> {
            Thread t = new Thread(r, "zerotrust-neoforge-scheduler");
            t.setDaemon(true);
            return t;
        });
